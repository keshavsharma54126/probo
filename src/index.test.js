...
import * as indexModule from '../dist/index';

describe("E-to-E-1", () => {
  beforeAll(async () => {
    await request(indexModule).post("/reset"); // resets the data values
  });

  it("this test just checks the response messages and status", async () => {
    // Step 1: Create a new user (User5)
    let response = await request(indexModule).post("/user/create/user5");
    expect(response.status).toBe(201);
    expect(response.body.message).toBe("User user5 created");

    // Step 2: Add balance to user5
    response = await request(indexModule).post("/onramp/inr").send({
      userId: "user5",
      amount: 50000,
    });
    ...
    response = await request(indexModule).post(
    ...
    response = await request(indexModule).post("/trade/mint").send({
      userId: "user5",
    ...
    response = await request(indexModule).post("/order/sell").send({
      userId: "user5",
      stockSymbol: "AAPL_USD_25_Oct_2024_14_00",
      quantity: 10,
      price: 1000,
    ...
    response = await request(indexModule).post("/user/create/user6");
    expect(response.status).toBe(201);
    ...
    response = await request(indexModule).post("/order/buy").send({
      userId: "user6",
      stockSymbol: "AAPL_USD_25_Oct_2024_14_00",
      quantity: 10,
      price: 1000,
    ...
    response = await request(indexModule).get("/balances/inr");
    expect(response.status).toBe(200);
...
describe("E-to-E-2", () => {
  ...
  it("this test checks the response values , status as well as state of the variables at regular intervals", async () => {
    ...
    let response = await request(indexModule).post("/user/create/user3");
    expect(response.status).toBe(201);
    ...
    response = await request(indexModule).get("/balances/stock");
    expect(response.status).toBe(200);
    ...
    response = await request(indexModule).post("/user/create/user4");
    expect(response.status).toBe(201);
...
describe("E-to-E-3", () => {
  beforeAll(async () => {
    await request(indexModule).post("/reset"); // Reset the data values
  });

  it("should handle multiple matching orders and price priorities correctly", async () => {
    // Step 1: Create users (User1 and User2)
    let response = await request(indexModule).post("/user/create/user1");
    expect(response.status).toBe(201);
    expect(response.body.message).toBe("User user1 created");

    response = await request(indexModule).post("/user/create/user2");
    expect(response.status).toBe(201);
    ...
    await request(indexModule).post("/order/sell").send({
      userId: "user1",
      stockSymbol: "ETH_USD_15_Oct_2024_12_00",
      quantity: 100,
      price: 1400,
    ...
    response = await request(indexModule).get("/orderbook");
    expect(response.status).toBe(200);
    ...
    response = await request(indexModule).post("/order/cancel").send({
      userId: "user1",
      stockSymbol: "ETH_USD_15_Oct_2024_12_00",
      quantity: 50,
      price: 1500,
  ...
  it("should handle multiple buy orders with price priority matching", async () => {
    ...
    await request(indexModule).post("/reset");

    // Step 1: Create users (User1 and User2)
    await request(indexModule).post("/user/create/user1");
    await request(indexModule).post("/user/create/user2");
    ...
    await request(indexModule).post("/symbol/create/ETH_USD_15_Oct_2024_12_00");
    await request(indexModule).post("/trade/mint").send({
      userId: "user1",
      stockSymbol: "ETH_USD_15_Oct_2024_12_00",
      quantity: 200,
      price: 1500,
    ...
    let response = await request(indexModule).get("/balances/stock");
    expect(response.status).toBe(200);
    expect(response.body["user1"]["ETH_USD_15_Oct_2024_12_00"]["yes"]).toEqual({
      quantity: 200,
      locked: 0,
...
describe("E-to-E-4", () => {
  ...
  it("should handle multiple buy orders with price priority matching when a third user introduces a matching sell price", async () => {
    ...
    await request(indexModule).post("/user/create/user1");
    await request(indexModule).post("/user/create/user2");
    await request(indexModule).post("/user/create/user3");
